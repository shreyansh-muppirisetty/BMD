"""Theme registry and resolution logic (ported from themes.js)."""
...
def resolve_theme(metadata=None, custom_themes=None):
    """Resolve a theme by name, applying custom themes, inheritance, accent and font overrides."""
    ...
    requested_name = metadata.get("theme") or "modern"
    requested = registry.get(requested_name) or registry["modern"]
    parent = None
    if requested.get("extends"):
        parent = registry.get(requested["extends"]) or registry["modern"]

    tokens = {}
    if parent:
        tokens.update(parent.get("tokens", {}))
    tokens.update(requested.get("tokens", {}))

    if metadata.get("accent"):
        tokens["accent"] = accent_colors.get(metadata["accent"], metadata["accent"])

    if metadata.get("font"):
        tokens["fontBody"] = font_tokens.get(metadata["font"], metadata["font"])
