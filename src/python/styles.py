"""CSS generation (ported from styles.js)."""

import re


def _to_kebab(value):
    return re.sub(r"[A-Z]", lambda m: f"-{m.group(0).lower()}", value)


def css_variables(theme):
    lines = [f"  --bmd-{_to_kebab(key)}: {value};" for key, value in theme["tokens"].items()]
    return "\n".join(lines)
